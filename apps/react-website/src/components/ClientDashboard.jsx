                    ...
                    {jobs.map(job => (
                        <li key={job.id} className="py-4 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                            ...
                            <div className="flex items-center gap-4">
                                <span className={`text-xs px-3 py-1 rounded-full uppercase font-bold tracking-wider ${job.status === 'in_progress' ? 'bg-blue-900/50 text-blue-300 border border-blue-500/30' : 'bg-green-900/50 text-green-300 border border-green-500/30'}`}>
                                    {job.status || 'Open'}
                                </span>
